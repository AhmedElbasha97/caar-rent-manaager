...
class _CarRequestDetailsScreenState extends State<CarRequestDetailsScreen> {
  ...
  String returnChosenDriverOrNot(){
    ...
    for (int i = 0; i < (widget.carData?.driver?.length ?? 0 ); i++) {
      theChosenOfWithDriverOrNot = "$theChosenOfWithDriverOrNot ${ Get
          ...
          .activeLocale ==
          SupportedLocales.english
          ? (listOfWithDriver?[int.parse((widget.carData?.driver?[i].isNotEmpty??false)?(widget.carData?.driver?[i]??"0"):"0")]?? "") : (listOfWithDriver?[int.parse((widget.carData?.driver?[i].isNotEmpty??false)?(widget.carData?.driver?[i]??"0"):"0")]?? "")}";
  ...
  Widget build(BuildContext context) {
    return GetBuilder(
      ...
      builder: (CarRequestDetailsController controller) =>  AnnotatedRegion<SystemUiOverlayStyle>(
