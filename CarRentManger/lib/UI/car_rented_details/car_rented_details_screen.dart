...
import '../editing_car_rent_requset/editing_car_rent_request_screen.dart';
...
class _CarRentedDetailsScreenState extends State<CarRentedDetailsScreen> {
  ...
  String returnChosenPeriods(){
    ...
    for (int i = 0; i < (widget.carData?.type?.length ?? 0); i++) {
      theChosenOfthePeriod = "$theChosenOfthePeriod ${ Get
          ...
          .activeLocale ==
          SupportedLocales.english
          ? (listOfPeriods?[int.parse((widget.carData?.type?[i].isNotEmpty??false)?(widget.carData?.type?[i]??"0"):"0")-1]?? "") : (listOfPeriods?[int.parse((widget.carData?.type?[i].isNotEmpty??false)?(widget.carData?.type?[i]??"0"):"0")-1]?? "")}";
  ...
  String returnChosenDriverOrNot(){
    ...
    for (int i = 0; i < (widget.carData?.driver?.length ?? 0 ); i++) {
      theChosenOfWithDriverOrNot = "$theChosenOfWithDriverOrNot ${ Get
          ...
          .activeLocale ==
          SupportedLocales.english
          ? (listOfWithDriver?[int.parse((widget.carData?.driver?[i].isNotEmpty??false)?(widget.carData?.type?[i]??"0"):"0")-1]?? "") : (listOfWithDriver?[int.parse((widget.carData?.driver?[i].isNotEmpty??false)?(widget.carData?.type?[i]??"0"):"0")-1]?? "")}";
  ...
  Widget build(BuildContext context) {
    return GetBuilder(
      ...
      builder: (CarRentedDetailsController controller) => AnnotatedRegion<SystemUiOverlayStyle>(
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                                children: [
                                                  ...
                                                  children: [
                                                    ...
                                                    Row(
                                                      children: [
                                                        ...
                                                        CustomText(
                                                          listInsuranceTypes?[(widget.carData?.incType ?? 1)-1]??"" ,
                                                          textAlign: TextAlign.center,
                                                          style:  TextStyle(
                                                            fontFamily: Get.find<StorageService>().activeLocale == SupportedLocales.english?fontFamilyEnglishName:fontFamilyArabicName,
                                                            color: kDarkBlueColor,
                                          ...
                                          child:Padding(
                                            ...
                                            child: Container(
                                              ...
                                              child:  Center(child: CustomText(
                                                Get.find<StorageService>().activeLocale == SupportedLocales.english?"End the rental process":"أنهاء عمليه التأجير",
                                                style:  TextStyle(
                                                  fontFamily: Get.find<StorageService>().activeLocale == SupportedLocales.english?fontFamilyEnglishName:fontFamilyArabicName,
                                                  color: kWhiteColor,
                                                  fontSize: 15,
                                  ...
                                  widget.carData?.edit==1?SizedBox():Center(
                                    child: InkWell(
                                      onTap: (){
                                       Get.to(()=>EditingCarRentRequestScreen(carData:widget.carData));
